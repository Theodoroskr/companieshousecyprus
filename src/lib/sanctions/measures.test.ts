...
describe("measures extraction", () => {
  ...
  describe("measureAvailability", () => {
    it("returns present when measures, note or amendment date exist", () => {
      ...
      expect(measureAvailability({ raw: { other_information: "Listed under Russia regime." } })).toBe("present");
    });

    it("returns not_published when the raw record exists but carries no measure fields", () => {
      expect(measureAvailability({ raw: { primary_name: "Meritservus HC Limited", designation_date: "2023-04-11" } })).toBe(
        "not_published",
      );
    });
