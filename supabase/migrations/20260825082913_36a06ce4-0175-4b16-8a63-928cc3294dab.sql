...
returns table (
  slug text, type_code text, name text, official_no text, reg_number text,
...
create or replace function public.companies_district_counts()
...
set search_path = public
as $$
  select district_en as name, count(*)::bigint
  from public.companies
  where district_en is not null
  group by district_en
  order by district_en;
$$;

grant execute on function public.companies_district_counts() to service_role;

create index if not exists companies_type_code_idx on public.companies (type_code);
